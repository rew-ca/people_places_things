...
module PeoplePlacesThings
  class StreetAddress
    ...
    #In addition to testing with production data, now using
    #using Abbreviations from Canada Post  http://www.canadapost.ca/tools/pg/manual/PGaddress-e.asp#1423617
    SUFFIXES = {
      :alley      => %w(alley aly ally allee al al.),
      :anex       => %w(anex anx annex annx anx.),
      :arcade     => %w(arcade arc arc.),
      :autoroute  => %w(autoroute aut),
      ...
      :burg       => %w(burg bg),
      :burgs      => %w(burgs bgs),
      :bypass     => %w(bypass byp bypa bypas byps by-pass),
      :camp       => %w(camp cp cmp),
      :canyon     => %w(canyon cyn canyn cnyn cyn.),
      :cape       => %w(cape cpe),
      :carrefour  => %w(carrefour carref),
      :carre      => %w(carré car),
      :causeway   => %w(causeway cswy causwa cswy.),
      :center     => %w(center ctr cent centr cnter centre cntr ctr.),
      :centers    => %w(centers ctrs centres),
      :chemin     => %w(chemin ch),
      :circle     => %w(circle cir circ circl crcl crcle cir.),
      :circuit    => %w(circuit circt),
      :cliff      => %w(cliff clf clf.),
      :cliffs     => %w(cliffs clfs),
      :club       => %w(club clb),
      :common     => %w(common cmn),
      :commons    => %w(commons cmns),
      :concession => %w(concession conc),
      :condo      => %w(condo con con.),
      :corner     => %w(corner cor),
      :corners    => %w(corners cors crnrs),
      :court      => %w(court crt ct ct. cor cor.),
      :courts     => %w(courts crts cts cts.),
      :cove       => %w(cove cv),
      :coves      => %w(coves cvs),
      :creek      => %w(creek crk crk.),
      :crescent   => %w(crescent cres crsent crsnt crs),
      :crest      => %w(crest crst),
      :croissant  => %w(croissant crois),
      :crossing   => %w(crossing xing xing. cross crssng crs crs.),
      :crossroad  => %w(crossroad xrd),
      :crossroads => %w(crossroads xrds),
      :cul_de_sac => %w(cul-de-sac cds),
      :curve      => %w(curve curv),
      :dale       => %w(dale dl),
      :dam        => %w(dam dm),
      :diversion  => %w(diversion divers),
      :divide     => %w(divide dv div dvd),
      :drive      => %w(drive dr driv drv dr.),
      :drives     => %w(drives drs),
      :echangeur  => %w(échangeur éch),
      :esplanade  => %w(esplanade espl),
      :estate     => %w(estate est),
      :estates    => %w(estates ests),
      :expressway => %w(expressway expy exp expr express expw expr),
      :extension  => %w(extension ext exten ext.),
      ...
      :grounds    => %w(grounds grnds),
      :grove      => %w(grove grv grov),
      :groves     => %w(groves grvs),
      :harbor     => %w(harbor hbr harb harbr hrbor),
      :harbors    => %w(harbors hbrs harbr),
      :haven      => %w(haven hvn),
      :heights    => %w(heights hts hts.),
      :highlands  => %w(highlands hghlds),
      :highway    => %w(highway hwy hy highwy hiway hiwy hway hwy. hgwy hgwy.),
      :hill       => %w(hill hl),
      :hills      => %w(hills hls),
      :hollow     => %w(hollow holw hllw hollows holw holws),
      :impasse    => %w(impasse imp),
      :inlet      => %w(inlet inlt),
      ...
      :lakes      => %w(lakes lks),
      :land       => %w(land land),
      :landing    => %w(landing lndg lndg. landng),
      :lane       => %w(lane ln ln.),
      :light      => %w(light lgt),
      :lights     => %w(lights lgts),
      :limits     => %w(limits lmts),
      :loaf       => %w(loaf lf),
      :lock       => %w(lock lck),
      :locks      => %w(locks lcks),
      :lodge      => %w(lodge ldg ldge lodg),
      :lookout    => %w(lookout lkout),
      ...
      :road       => %w(road rd rd.),
      :roads      => %w(roads rds),
      :rond_point => %w(ront_point rond-point rdpt),
      :route      => %w(route rte),
      :row        => %w(row row),
      :rue        => %w(rue rue),
      :ruelle     => %w(ruelle rle),
      :run        => %w(run run),
      :sentier    => %w(sentier sent),
      :shoal      => %w(shoal shl),
      ...
      :subdivision => %w(subdivision subdiv),
      :summit     => %w(summit smt sumit sumitt),
      :terrace    => %w(terrace ter terr ter. te te.),
      :terrasse   => %w(terrasse tsse),
      :thicket    => %w(thicket thick),
      :throughway => %w(throughway trwy),
      :townline   => %w(townline tline),
      ...
      :view       => %w(view vw),
      :views      => %w(views vws),
      :village    => %w(village villge vlg vill villg villiage vlg),
